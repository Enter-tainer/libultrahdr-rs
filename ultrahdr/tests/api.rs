...
// ---------------------------------------------------------------------------------------------
// HEIF / AVIF containers (`heif` feature)
// ---------------------------------------------------------------------------------------------

/// HEIF/AVIF output is only defined for raw intents, so both intents are supplied as raw images.
#[cfg(feature = "heif")]
mod heif_containers {
    use super::*;

    fn container_stream(codec: Codec) -> Vec<u8> {
        encode_configured(&hdr_image(32, 32), Some(&sdr_image(32, 32)), |enc| {
            enc.set_output_format(codec)
        })
    }

    fn assert_round_trip(codec: Codec, brand: &[u8]) {
        let stream = container_stream(codec);
        assert_eq!(
            &stream[4..8],
            b"ftyp",
            "{codec:?} output is not an ISO BMFF file"
        );
        assert_eq!(&stream[8..12], brand, "{codec:?} major brand");
        assert!(is_uhdr_image(&stream), "{codec:?} is not a gain map image");

        let mut dec = Decoder::new().expect("create decoder");
        set_stream(&mut dec, &stream);
        assert_eq!(
            (
                dec.image_width().expect("width"),
                dec.image_height().expect("height")
            ),
            (32, 32)
        );
        if let Err(error) = dec.decode() {
            // Encoding only needs a HEVC/AV1 encoder; decoding needs a matching decoder plugin,
            // which a host may not have installed.
            assert!(
                matches!(&error, Error::Codec(detail) if detail.contains("No decoding plugin")),
                "{codec:?} decode failed: {error:?}"
            );
        }
    }

    #[test]
    fn avif_round_trips_with_a_gain_map() {
        assert_round_trip(Codec::Avif, b"avif");
    }

    #[test]
    fn heif_round_trips_with_a_gain_map() {
        assert_round_trip(Codec::Heif, b"heic");
    }

    fn multichannel_encode(codec: Codec, width: u32, height: u32) -> Result<Vec<u8>, Error> {
        let mut enc = Encoder::new().expect("create encoder");
        enc.set_output_format(codec)?;
        enc.set_multi_channel_gainmap(true)?;
        enc.set_raw_image(ImageLabel::Hdr, &hdr_image(width, height))?;
        enc.set_raw_image(ImageLabel::Sdr, &sdr_image(width, height))?;
        enc.encode()?;
        Ok(enc
            .encoded_stream()
            .expect("encoded stream")
    ...
    /// Upstream walks off the end of an odd-height multi-channel gain map when it converts it to
    /// YCbCr for a container; the wrapper has to reject that instead of aborting the process.
    #[test]
    fn container_multichannel_with_odd_gain_map_height_is_rejected() {
        for codec in [Codec::Avif, Codec::Heif] {
            let error = multichannel_encode(codec, 32, 33).expect_err("odd gain map height");
            assert!(matches!(error, Error::UnsupportedFeature(_)), "{error:?}");
        }

        // An even gain map height is fine.
        for codec in [Codec::Avif, Codec::Heif] {
            multichannel_encode(codec, 32, 34).expect("even gain map height");
        }
    }

    /// JPEG output takes a different (safe) path, so the same shape keeps working there.
    #[test]
    fn jpeg_multichannel_with_odd_height_still_encodes() {
        let stream = encode_configured(&hdr_image(32, 33), Some(&sdr_image(32, 33)), |enc| {
